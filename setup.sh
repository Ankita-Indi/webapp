...
sudo mysql --connect-expired-password -u root -p$passwords -e "ALTER USER 'root'@'localhost' IDENTIFIED BY 'Pratappk@890'"
...
# sudo mysql -uroot -p$pwd --connect-expired-password -e "Alter user 'root'@'localhost' IDENTIFIED 
 #mysql_native_password BY 'Pratappk@890'"
 mysql -uroot -pPratappk@890  -e "CREATE DATABASE IF NOT EXISTS cloud"

pwd
# sudo mv health-check-api-0.0.1-SNAPSHOT.jar /tmp/
...
# sudo chmod 544 health-check-api-0.0.1-SNAPSHOT.jar
# sudo mkdir webapp

# sudo mv health-check-api-0.0.1-SNAPSHOT.jar /usr/bin/
